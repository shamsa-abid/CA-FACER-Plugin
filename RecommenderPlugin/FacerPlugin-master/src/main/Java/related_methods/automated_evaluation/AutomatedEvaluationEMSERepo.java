 ...
 */
public class AutomatedEvaluationEMSERepo {
	...
	public static void main(String args[]) throws Exception
	{
		...
		//metrics.add(getRecStats(63, 14235, minSup));
		//metrics.add(getRecStats(9, 3605, minSup));
		metrics.add(getRecStats(13, 4721, minSup));
	...
	private static Metrics getRecStats(int pid, int MID, int minSup) throws Exception
	{
		...
		if(relatedFeatureIDs.size()>0 && groundTruthFeatureIDs.size()>0)
		{
			if(relatedFeatureIDs.size()>30)
				relatedFeatureIDs.subList(30, relatedFeatureIDs.size()).clear();
