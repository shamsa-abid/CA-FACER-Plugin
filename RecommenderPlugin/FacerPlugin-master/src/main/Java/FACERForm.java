...
import static groovy.inspect.Inspector.JAVA;
    ...
    private MouseAdapter queryMethodsListMouseAdapter = new MouseAdapter() {
        public void mouseClicked(MouseEvent evt) {
            ...
            if (evt.getClickCount() == 2 && evt.getButton() == MouseEvent.BUTTON1) {
                ...
                try {
                    method.apiCallsList = FACERSearchService.getInstance().getAPICalls(method.id);
                } catch (Exception e) {
                    e.printStackTrace();
                }
...
//                getRelatedMethodsForMethod(method);
    ...
    public void populateContextRelatedMethods(Method queryMethod, Object[] results) {
        relatedMethodsList.setListData(results);
        // event 15
        EventLoggerService.getInstance().log(15,
                new ArrayList<String>(Arrays.asList(
                        "method_id:" + queryMethod.id,
                        "method_name:" + queryMethod.name,
                        "related_algo:" + queryMethod.algo,
    ...
    private void getContextRelatedMethodsForMethod(Method method) {
        //      event 4
        EventLoggerService.getInstance().log(15, new ArrayList<String>(Arrays.asList(
                "method_id:" + method.id,
                "method_name:" + method.name)));
        ArrayList relatedMethods = FACERSearchService.getInstance().getContextRelatedMethods(method.id);
        /*if(!relatedMethods.isEmpty()){
            method.algo = FACERSearchService.getInstance().getAlgoForRelatedMethodAt(0);
        }*/
        method.algo = "context algo";
        populateContextRelatedMethods(method, relatedMethods.toArray());
    }
    ...
    public void showMethodBody(Method method){
        ...
        JButton getContextRelatedMethodsButton = new JButton("CACR recs");
        getContextRelatedMethodsButton.setBorder(BorderFactory.createEmptyBorder());
        getContextRelatedMethodsButton.setPreferredSize(new Dimension(20,20));
        getContextRelatedMethodsButton.setToolTipText("Search contextually relevant methods");
        getContextRelatedMethodsButton.addActionListener(evt -> {
            getContextRelatedMethodsForMethod(method);
        });
...
//        });
//shamsa added line below
        final Project project = ProjectUtil.guessCurrentProject(mainPanel);
        Document document2 = new DocumentImpl(method.body, true, false);
        Editor windowEditor =
                EditorFactory.getInstance().createEditor(
                        document2, project, FileTypeManager.getInstance()
...
//        textArea.setEditable(true);
        JScrollPane scrollableTextArea = new JBScrollPane(windowEditor.getComponent());
        //JScrollPane scrollableTextArea = new JBScrollPane(textArea);

        //windowEditor.getDocument().setText(method.body);
        TextAttributes ta1 =  new TextAttributes();
        ta1.setForegroundColor(Color.darkGray);
        ta1.setBackgroundColor(Color.LIGHT_GRAY);

        TextAttributes ta2 =  new TextAttributes();
        ta2.setForegroundColor(Color.darkGray);
        ta2.setBackgroundColor(Color.ORANGE);

        if(GetContextAction.editor != null) {
            Document document = GetContextAction.editor.getDocument();
            //windowEditor.getMarkupModel().addLineHighlighter(10,1,ta);
            GetContextAction.editor.getMarkupModel().removeAllHighlighters();


            for (String s : method.apiCallsList) {
                int separatorIndex = s.indexOf(".");
                String apiname = s.substring(0, separatorIndex);
                String apicall = s.substring(separatorIndex + 1);
                int startIndex = method.body.indexOf(apiname);
                int sIndex = document.getText().indexOf(apiname);
                if (startIndex != -1) {


                    int endIndex = startIndex + apiname.length();
                    windowEditor.getMarkupModel().addRangeHighlighter(startIndex, endIndex, 1, ta1, EXACT_RANGE);

                    //GetContextAction.editor.getMarkupModel().addRangeHighlighter(sIndex, endIndex, 1, ta1, EXACT_RANGE);
                }
                if (sIndex != -1) {
                    int endIndex = sIndex + apiname.length();
                    //windowEditor.getMarkupModel().addRangeHighlighter(startIndex, endIndex, 1, ta1, EXACT_RANGE);
                    GetContextAction.editor.getMarkupModel().addRangeHighlighter(sIndex, endIndex, 1, ta1, EXACT_RANGE);


                }
                //ta.setForegroundColor(Color.white);

                startIndex = method.body.indexOf(apicall);
                sIndex = document.getText().indexOf(apicall);
                if (sIndex != -1) {

                    int endIndex = sIndex + apicall.length();
                    //windowEditor.getMarkupModel().addRangeHighlighter(startIndex, endIndex, 1, ta2, EXACT_RANGE);
                    GetContextAction.editor.getMarkupModel().addRangeHighlighter(sIndex, endIndex, 1, ta2, EXACT_RANGE);
                }
                if (startIndex != -1) {

                    int endIndex = startIndex + apicall.length();
                    windowEditor.getMarkupModel().addRangeHighlighter(startIndex, endIndex, 1, ta2, EXACT_RANGE);
                    //GetContextAction.editor.getMarkupModel().addRangeHighlighter(sIndex, endIndex, 1, ta2, EXACT_RANGE);
                }
            }

            //highlighting text in main editing window
        }
        ...
        if(sIndex!=-1) {
            ...
            int endIndex = sIndex + "openFile".length();
            GetContextAction.editor.getMarkupModel().addRangeHighlighter(sIndex, endIndex, 1, ta, EXACT_RANGE);
        }*/
        //GetContextAction.editor.getMarkupModel().addLineHighlighter(10,1,ta);



        JPanel optionsWrapper = new JPanel(new FlowLayout(FlowLayout.RIGHT));
        optionsWrapper.add(getRelatedMethodsButton);
        optionsWrapper.add(getContextRelatedMethodsButton);
    ...
    public void showCodeFileBody(Method method, CodeFile codeFile){
        ...
        final Project project = ProjectUtil.guessCurrentProject(mainPanel);
        Document document2 = new DocumentImpl(codeFile.body, true, false);
        Editor windowEditor =
                EditorFactory.getInstance().createEditor(
                        document2, project, FileTypeManager.getInstance()
                                .getFileTypeByExtension(JAVA),
